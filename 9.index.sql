...
-- index는 한개의 컬럼 뿐만 아니라, 두개의 컬럼을 대상으로 1개의 index를 설정하는 것도 가능.
-- 이 경우 두 컬럼을 and조건으로 조회해야만 index를 사용(복합 index)
create index 인덱스명 on 테이블명(컬럼1, 컬럼2);
create index name_age_index on author(name, age);

-- index 성능테스트
-- 기존 테이블 삭제 후 간단한 테이블로  index설정 또는 index미설정 테스트
create table author(id bigint auto_increment, email varchar(255), name varchar(255), primary key(id));  -- 테이블 생성
...
BEGIN
    ...
    DECLARE max_iterations INT DEFAULT 100; -- 총 반복 횟수 (1000000 / batch_size) 천만건
